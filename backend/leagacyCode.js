...
//     const filename = req.headers['x-file-name'];
//     if (!filename) return res.status(400).send("Missing x-file-name header");

//     // Define supported file types
//     const videoExtensions = ['.mp4', '.avi', '.mov', '.mkv'];
//     const documentExtensions = ['.pdf', '.docx', '.txt', '.jpg', '.jpeg', '.png', '.doc', '.xls', '.xlsx'];
...
//     const tempInputFile = path.join(tempDir, `input-${Date.now()}${fileExt}`);
//     let tempOutputFile = tempInputFile; // Default for documents
//     const s3Key = `${isVideo ? 'videos' : 'documents'}/${Date.now()}-${filename}`;

//     // Step 1: Save incoming chunks to temporary file
...
//     // Step 2: Process videos with FFmpeg (skip for documents)
//     if (isVideo) {
//       tempOutputFile = path.join(tempDir, `output-${Date.now()}.mp4`);
//       await new Promise((resolve, reject) => {
//         ffmpeg(tempInputFile)
//           .videoCodec('libx264')
//           .audioCodec('aac')
...
//           .outputOptions([
//             '-movflags faststart',
//             '-preset ultrafast', // Optimized for large files
//             '-crf 23',
//           ])
//           .save(tempOutputFile)
//           .on('end', () => {
...
//     // Step 4: Upload file in chunks (use read stream for memory efficiency)
//     const readStream = createReadStream(tempOutputFile, { highWaterMark: CHUNK_SIZE });
//     let buffer = Buffer.alloc(0);
...
//     await Promise.all([
//       fs.unlink(tempInputFile).catch((err) => console.error('Failed to delete input file:', err)),
//       isVideo ? fs.unlink(tempOutputFile).catch((err) => console.error('Failed to delete output file:', err)) : Promise.resolve(),
//     ]);

//     res.json({ contentId: content._id, key: s3Key });
//   } catch (err) {
//     console.error("Upload error:", err);

//     // Clean up on error
//     const tempDir = os.tmpdir();
//     const tempInputFile = path.join(tempDir, `input-${Date.now()}${fileExt || '.tmp'}`);
//     const tempOutputFile = path.join(tempDir, `output-${Date.now()}.mp4`);
//     await Promise.all([
//       fs.unlink(tempInputFile).catch(() => {}),
//       isVideo ? fs.unlink(tempOutputFile).catch(() => {}) : Promise.resolve(),
//     ]);
...
// The unified handler works for both GET and HEAD
// -------------------------------
// 🔹 Session Handling
// -------------------------------
...
//       const sessionKey = `session:${sessionId}`;
//       const sessionData = await redisClient.get(sessionKey);
//       console.log('Session lookup:', { sessionId, sessionData });
//       if (sessionData) {
//         const session = JSON.parse(sessionData);
//         const nowSeconds = Math.floor(Date.now() / 1000);

//         if (session.contentId !== contentId) return res.status(403).send('Forbidden');

//         if (nowSeconds >= session.exp) {
//           // 🔹 Session expired, but try to refresh via token
//           if (!token) {
//             res.setHeader('x-session-expired', 'true');
//             return res.status(440).send('Session expired');
//           }
...
//           const now = Math.floor(Date.now() / 1000);
//           const ttl = Math.min(10, exp - now); // 10 mins or remaining token life

//           await redisClient.setEx(newSessionKey, ttl, JSON.stringify({ role, contentId, exp }));
//           res.setHeader('x-session-id', newSessionId);  // 🔹 inform client about new session
//         } else {
//           role = session.role;
//           exp = session.exp;
...
//         const newSessionId = uuidv4();
//         await redisClient.setEx(`session:${newSessionId}`, ttl, JSON.stringify({ role, contentId, exp }));
//         res.setHeader('x-session-id', newSessionId);
//       }
//     } else {
//       // 🔹 First request with only token
//       if (!token) return res.status(401).send('Unauthorized');
...
//     // 🔹 Role validation
//     if (!content.allowedRoles.includes(role)) return res.status(403).send('Forbidden');
...
//     const headRes = await s3.headObject({ Bucket: bucketName, Key: content.s3Key }).promise();
//     const fileSize = headRes.ContentLength;
//     const finalEnd = Math.min(end, fileSize - 1);
//     const contentLength = finalEnd - start + 1;
...
//       await redisClient.setEx(sessionKey, 10, JSON.stringify({
//         role,
//         contentId,
//       })); // 5-minute expiration
...
//     const match = range.match(/bytes=(\d+)-(\d*)/);
//     const start = parseInt(match[1], 10);
//     const end = match[2] ? parseInt(match[2], 10) : start + 2*10 ** 6; // 2MB default chunk
